...
_Dieses Dokument ist vertraulich und ausschließlich für den internen Gebrauch bestimmt._

ABSCHNITT 1

**Projektübersicht**

Ziele, Stakeholder, Projektvision und Rahmenbedingungen
...
Funktionale & nicht-funktionale Anforderungen, User Stories, Use Cases
...
Tech-Stack, Datenbankschema, API-Design, UI/UX-Konzept
...
Da keine Tech-Stack-Praeferenz angegeben wurde, wird folgender moderner, produktionserprobter Stack empfohlen, der sich fuer Teams jeder Groesse und einen Zeitrahmen von 3-6 Monaten eignet.
...
| ----------------- | ----------------------------------------------------------------------------------------------------------- | -------------------------------------- |
| users             | id, email, phone, role, created_at, avatar_url                                                              | Alle Nutzer (Auszubildende + Betriebe) |
| companies         | id, user_id, name, slug, description, industry, location, logo_url, header_url, custom_theme_json, verified | Ausbildungsbetriebe                    |
| reviews           | id, company_id, user_id, is_anonymous, weight (0.5/1.0), overall_score, created_at                          | Bewertungskoepfe                       |
| review_answers    | id, review_id, question_id, star_value (null\|1-5), text_value (null\|text)                                 | Einzelne Fragenantworten               |
| review_categories | id, name, sort_order                                                                                        | 5 Bewertungskategorien                 |
| review_questions  | id, category_id, text, type (stars\|text), sort_order                                                       | 25 Bewertungsfragen                    |
| review_comments   | id, review_id, company_id, text, created_at                                                                 | Betriebsantworten auf Bewertungen      |
| job_listings      | id, company_id, title, description, location, start_date, requirements, is_active                           | Ausbildungsstellenanzeigen             |
| applications      | id, job_id, user_id, status (new/reviewing/rejected/invited), cv_url, cover_url                             | Bewerbungen                            |
| review_helpful    | review_id, user_id, created_at                                                                              | Hilfreich-Markierungen                 |

## 3.4 Bewertungs-Gewichtungsformel

**Berechnungslogik**

Durchschnittsbewertung = SUMME(review.weight \* review.overall_score) / SUMME(review.weight) Dabei gilt: Anonyme Bewertung -> weight = 0.5 | Namentliche Bewertung -> weight = 1.0 Die overall_score ergibt sich aus dem Durchschnitt aller Sterne-Fragen der 25 Bewertungsfragen (Textfragen fliessen nicht in den Score ein).
...
Sprint-Planung, Meilensteine, Implementierungsreihenfolge
...
Die Suche wird als Postgres Full-Text-Search implementiert (tsvector/tsquery) mit deutschem Woerterbuch (pg_catalog.german). Suchergebnisse werden debounced (300 ms) abgerufen. Bei mehr als 3 Zeichen Eingabe werden Live-Vorschlaege angezeigt. Ergebnisse werden nach Relevanzscore und Anzahl der Bewertungen sortiert.

### 4.3.4 Komponentenarchitektur (Frontend)

Da Karriko eine wachsende Plattform mit dynamischen Inhalten ist (Nutzerprofile, Unternehmensseiten, Bewertungskarten), wird eine strikte Komponentenarchitektur eingesetzt. Ziel ist es, Layout-Elemente wie Navbar, Footer oder Bewertungskarten genau einmal zu programmieren und auf allen Seiten wiederzuverwenden. In Next.js erfolgt dies ueber den /components-Ordner.

**Ordnerstruktur**

/components/shared/ → Navbar, Footer, Avatar, Modal

/components/review/ → ReviewCard, StarRating, ReviewForm

/components/company/ → CompanyHeader, CompanyCard, ProfileEditor

/components/jobs/ → JobCard, ApplicationForm, StatusBadge

**Server Components vs. Client Components**

Next.js 14 unterscheidet zwischen Server Components (standardmaessig, kein „use client"-Direktive) und Client Components (interaktiv, mit State). Fuer Karriko gilt folgende Aufteilung: Statische oder SEO-relevante Inhalte wie Betriebsprofile und Bewertungsseiten werden als Server Components ausgefuehrt, da sie direkt auf die Datenbank zugreifen und HTML vorgerendert ausliefern. Interaktive Elemente wie das Suchfeld mit Autocomplete, das Bewertungsformular und der Profil-Editor werden als Client Components implementiert.

**Dynamische Daten in Komponenten**

Datenbankabfragen werden in Next.js Server Components direkt mit Prisma ausgefuehrt und die Daten als Props an untergeordnete Komponenten weitergegeben. Fuer Client Components, die nach dem initialen Laden Daten benoetigen (z.B. Live-Suchergebnisse), werden tRPC-Hooks eingesetzt (z.B. trpc.company.search.useQuery). Jede Komponente ist damit eigenstaendig und austauschbar, ohne dass Seiten-Templates angepasst werden muessen.

**Layout-Komponenten (geteilte Elemente)**

Elemente wie Navbar und Footer, die auf jeder Seite erscheinen, werden einmalig in der Datei app/layout.tsx definiert. Next.js wendet dieses Root-Layout automatisch auf alle Seiten an, ohne dass es auf jeder Seite einzeln eingebunden werden muss. Seitenspezifische Layouts (z.B. Dashboard mit Seitenleiste) werden als verschachtelte Layouts in den jeweiligen Routenordnern abgelegt (z.B. app/dashboard/layout.tsx).
...
Teststrategie, Testfaelle, Testumgebungen
...
Infrastruktur, CI/CD, Skalierung, Monitoring
...
- Lint & Type Check (ESLint, TypeScript) - Abbruch bei Fehlern
- Unit & Integrationstests (Vitest) - Abbruch bei Testfehlern
- Build (next build) - Abbruch bei Build-Fehlern
- Preview Deployment auf Vercel (automatische URL)
- E2E-Tests (Playwright) gegen Preview Deployment
- Security Scan (npm audit --audit-level=high)

Bei Merge in main zusaetzlich:

- Datenbank-Migration auf Produktion (prisma migrate deploy)
- Production Deployment auf Vercel
- Smoke-Tests gegen Produktion
- Slack/E-Mail-Notification an Team
...
DSGVO-Compliance, OWASP, Anonymität, Datenschutz
...
**Technische Umsetzung der Anonymitaet**

Beim Speichern einer anonymen Bewertung wird die user_id in der Datenbank hinterlegt (fuer interne Missbrauchspruefung und Eindeutigkeitskontrolle), aber das is_anonymous-Flag gesetzt. Die API-Endpunkte und das Frontend geben bei anonymen Bewertungen niemals Nutzernamen, Avatar oder andere identifizierende Daten zurueck. Die user_id ist ausschliesslich fuer Admins zur Moderation sichtbar.

ABSCHNITT 8

**Wartung & Weiterentwicklung**

Betrieb, SLA, Roadmap, Change Management
...
Gesamtplanung, Teambesetzung, Risikoregister
...
_SDLC · Karriko · Version 1.0 · Marz 2026_